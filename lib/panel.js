...
if (!require("sdk/sdk/api-utils/xul-app").is("Firefox")) {
  throw new Error([
...
const { Cc, Ci } = require("sdk/chrome");

const { validateOptions: valid } = require("sdk/api-utils/api-utils");
const { Symbiont } = require("sdk/api-utils/content");
const { EventEmitter } = require('sdk/api-utils/events');
const timer = require("sdk/api-utils/timer");
const runtime = require("sdk/api-utils/runtime");
...
const Panel = Symbiont.resolve({
  ...
  show: function show(anchor,where) {
    console.log("Gregg panel");
    anchor = anchor || null;
    ...
    if (!xulPanel) {
      ...
      let css = ".panel-inner-arrowcontent, .panel-arrowcontent {padding: 0; background:transparent; box-shadow: none}";
    ...
    function waitForBinding() {
      ...
      console.log("where", anchor, position, x,y);
